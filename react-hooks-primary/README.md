...
# ForwardRef
ForwardRef란 부모컴포넌트에서 자녀컴포넌트로 ref 전달할때 사용하는 기법이다. 특히 부모컴포넌트가 자녀컴포넌트에 접근해야할때 유용하게 쓰인다.

## 사용법
- 부모 컴포넌트 : ref를 전달할 자식컴포넌트에 key: 'ref', value: 'useRef로 등록한 값'를 전달한다.
![34.png](./study/34.png)  

- 자식 컴포넌트 : forwardRef를 import 해와서 컴포넌트를 감싼다. 그러면 해당컴포넌트는 두개의 인자를 받을 수 있다. 그 중 두번째가 부모가 전달할 ref가 된다.

![35.png](./study/35.png)

혹은

- 마지막 부분에 export default forwardRef(자식 컴포넌트)로 해주어도 위와 동일하다
![36.png](./study/36.png)  

c.f) forwardRef는 react 16.3 이전 버전에서는 지원하지 않았다. 그렇다면 그 이전에는 ref 전달을 어떻게 헀을까

- 부모 컴포넌트 : 16.3 이전버전에서는 자식 컴포넌트에 'ref'라는 이름으로 key를 지정할 수 없었기에, 다른 이름으로 명명했었다.
![37.png](./study/37.png)  

- 자식 컴포넌트 : 부모 컴포넌트가 전달한 ref 이름을 받아서 ref을 등록한다.
![38.png](./study/38.png)  

### 주의! 분명 유용한 기능이지만 너무 남발하지 말자. forwardRef를 사용하여 지나친 유연성을 부여하면 자식 컴포넌트의 캡슐화를 막는다. 
![39.png](./study/39.png)  