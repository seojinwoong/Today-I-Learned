...
### props, state와 관계없이 부모컴포넌트가 렌더링된다면 그 안의 자식컴포넌트들의 자동적으로 리랜더링된다.
...
- react.memo로 컴포넌트를 감싸면, 해당 컴포넌트는 Prop Check를 하게 된다.
  ...
  반대로 prop으로 받은 값이 변한다면 재렌더링이 된다.
![23.png](./study/23.png)  

- React.memo는 분명 성능을 최적화하는 좋은 도구이지만 남발하면 안된다. 렌더링된 컴포넌트의 결과를 따로 저장하는 것이기 때문이다.
![24.png](./study/24.png)  

## useMemo + React.memo
- React.memo로 자식컴포넌트를 감싸주어도 계속해서 리랜더링되는 경우가 있다. 그것은 부모컴포넌트의 값의 자료형이 참조형일때이다. 그럴때는 useMemo를 사용하여 값을 기억해서 참조형 자료도 값이 변하지 않는다고 작업해주어야 한다.
